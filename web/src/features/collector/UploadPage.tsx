...
export function UploadPage({ navigate }: { navigate?: (path: string) => void }) {
  ...
  // 从「任务卡」进入提交时，自动绑定该任务卡所属的采集任务（sceneTypeTaskId）
  const boundTask =
    tasks.find((task) => task.id === guideTask?.sceneTypeTaskId) ?? null;
  // 可从「任务卡」进入提交（sceneLibraryId + guideTaskId），此时无需选采集任务
  const fromGuideTask = Boolean(guideTask?.id && guideTask.sceneLibraryId);
  // 步骤 1「任务卡操作提示」完成：已确定任务归属（任务卡自动绑定，或已选任务并确认要求）
  const step1Complete =
    fromGuideTask || Boolean(selectedTask && taskRequirementsConfirmed);
  // 步骤 2「确认规范」完成：授权/隐私/敏感三项均已确认
  const step2Complete =
    authorization.dataUsageAuthorized &&
    authorization.privacyConfirmed &&
    authorization.sensitiveContentConfirmed;
  const authorizationComplete = step1Complete && step2Complete;
  // 上传前还需完成的步骤（用于第 3 步的引导文案与「前往」按钮）
  const pendingStep: 1 | 2 = step1Complete ? 2 : 1;
  const pendingLabel = pendingStep === 1 ? "选择任务" : "确认规范";
  ...
  function agreeAllAuthorization() {
    setAuthorization({
  ...
  return (
    <div className="page-stack">
      ...
      <div className="upload-wizard-steps" role="tablist" aria-label="提交步骤">
        <button
          type="button"
          role="tab"
          aria-selected={currentStep === 1}
          className={`upload-wizard-step${currentStep === 1 ? " active" : ""}${step1Complete ? " complete" : ""}`}
          onClick={() => setCurrentStep(1)}
        >
          <span className="upload-wizard-step-num">{step1Complete ? <CheckCircle2 size={15} /> : "1"}</span>
          <div><strong>任务卡操作提示</strong><small>确认任务归属</small></div>
        </button>
        <button
          type="button"
          role="tab"
          aria-selected={currentStep === 2}
          className={`upload-wizard-step${currentStep === 2 ? " active" : ""}${step2Complete ? " complete" : ""}`}
          onClick={() => setCurrentStep(2)}
        >
          <span className="upload-wizard-step-num">{step2Complete ? <CheckCircle2 size={15} /> : "2"}</span>
        ...
        <button
          ...
          className={`upload-wizard-step${currentStep === 3 ? " active" : ""}${uploads.length > 0 ? " complete" : ""}`}
          onClick={() => setCurrentStep(3)}
        >
          <span className="upload-wizard-step-num">{uploads.length > 0 ? <CheckCircle2 size={15} /> : "3"}</span>
      ...
      <section className="upload-layout">
        <div className="content-card upload-main-card upload-flow-card">
          ...
          {currentStep === 1 && (
            <section className="upload-step-card wizard-pane" aria-label="任务卡操作提示">
              <div className="upload-step-heading">
                <span>{step1Complete ? <CheckCircle2 size={16} /> : "1"}</span>
                <div><h2>任务卡操作提示</h2><p>{fromGuideTask ? "已从任务卡进入，确认任务卡内容后进入下一步。" : "选择你的采集任务，并阅读任务要求。"}</p></div>
              </div>
              <div className="upload-step-body">
                {fromGuideTask ? (
                  <div className="upload-guide-prompt-stack">
                    <div className="task-requirements-preview guide-task-bound">
                      <p className="task-req-title">
                        <strong>{guideTask?.title ?? `任务卡 ${(guideTask?.taskIndex ?? 0) + 1}`}</strong>
                        <span>{boundTask ? `场景：${boundTask.sceneName}` : "任务卡采集"}</span>
                      </p>
                      <p className="task-req-desc">
                        已从「任务大厅 → 去采集」进入，本次视频直接归属该采集任务，无需再手动选择任务。
                      </p>
                    </div>
                    {guideTask?.taskCard && (
                      <div className="upload-guide-prompt">
                      ...
                      >
                        ...
                        {tasks.map((task) => (
                          <option key={task.id} value={task.id} disabled={task.status !== "published"}>
                            ...
                            {task.status !== "published" ? " · 已暂停" : task.pricePerHour !== null ? ` · ${task.pricePerHour} 元/小时` : ""}
                          </option>
                        ))}
                      </select>
                    </label>
          ...
          {currentStep === 2 && (
            <section className="upload-step-card wizard-pane" aria-label="确认授权与隐私规范">
              <div className="upload-step-heading">
                <span>{step2Complete ? <CheckCircle2 size={16} /> : "2"}</span>
                <div><h2>确认授权与隐私规范</h2><p>三项都需要确认，避免上传无授权或包含敏感信息的视频。</p></div>
              </div>
              <div className="upload-step-body upload-consent-panel">
                <div className="upload-consent-agree">
                  <button
                    type="button"
                    className={`button ${step2Complete ? "button-secondary" : "button-primary"}`}
                    aria-label={step2Complete ? "已同意全部规范" : "一键同意全部规范"}
                    onClick={agreeAllAuthorization}
                  >
                    <CheckCircle2 size={15} />
                    {step2Complete ? "已同意全部规范" : "一键同意全部规范"}
                  </button>
                  <span>
                    {step2Complete
                      ? "您已确认：数据使用授权 · 隐私信息检查 · 敏感内容处理"
                      : "点击后自动勾选并确认下方三项规范"}
                  </span>
                </div>
          ...
          {currentStep === 3 && (
            <section className="upload-step-card wizard-pane" aria-label="上传视频文件">
              <div className="upload-step-heading">
                <span>3</span>
              ...
              <div className="upload-step-body">
                <button
                  ...
                  type="button"
                  aria-label={authorizationComplete ? undefined : "请先确认任务与授权规范"}
                ...
                >
                  ...
                  <strong>{authorizationComplete ? "点击选择或拖拽视频到这里" : "请先确认任务与授权规范"}</strong>
                  <small>MOV、MP4 格式 · 单文件最大 2 GiB · 支持批量上传</small>
                  <em>{authorizationComplete ? "选择视频文件" : `前往「${pendingLabel}」步骤，完成后再来上传`}</em>
                </button>
                {!authorizationComplete && (
                  <button
                    type="button"
                    className="button button-secondary button-small upload-dropzone-goto"
                    onClick={() => setCurrentStep(pendingStep)}
                  >
                    前往{pendingLabel}
                  </button>
                )}
        ...
        <aside className="content-card upload-guide-card upload-sticky-guide">
          {(boundTask ?? selectedTask) && (
            <div className="selected-task-summary">
              <small>本次任务来源</small>
              <strong>{(boundTask ?? selectedTask)!.title}</strong>
              <span>{(boundTask ?? selectedTask)!.sceneName} · V{(boundTask ?? selectedTask)!.revision}</span>
              <em>{(boundTask ?? selectedTask)!.pricePerHour !== null ? `${(boundTask ?? selectedTask)!.pricePerHour} 元/小时` : "按全局规则计费"}</em>
            </div>
          )}
