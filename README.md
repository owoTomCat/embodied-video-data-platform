...
- 管理员在 `/admin/withdrawals` 工作台领取申请，原子指定经办人并进入 processing。领取不要求导出，更不会自动付款。显式交接须填写原因并记录旧 / 新经办人；旧经办人不能继续登记。
- 显式查看完整收款信息会留审计；处理中仅当前经办人可查看或导出。CSV 仅辅助核对，**导出不是付款**，平台不连接银行，不声称已经验证真实到账。
- 实际转账后，经办人登记参考号、实际转账时间和 1–5 个凭证（JPEG/PNG/PDF，每份最多 5 MiB），进入 review_pending，预留资金不动。登记不可编辑或删除，退回后的更正追加历史。同渠道跨申请参考号不可复用。
- 正常采用双人复核：复核人不能是当前经办人，也不能是本申请任何历史登记人，交接或追加登记不能绕过此限制。仅恰有一名有效管理员时，允许显式选择 single，记录“单人确认，非独立复核”，绝不冒充双人保障。通过后预留原子转为已提现 / 累计提现，只产生一笔 withdraw 流水。终态不可反转。
- 复核退回或银行结果不明进入 investigating，保持全部预留；**退回不是重新付款指令**。应先查银行流水、参考号及退回事实，禁止重复转账或自动退款。只有实际确认未转账 / 资金已退回，填写原因、确认并提供凭证，才能 resolve-unpaid 释放预留；有过付款登记时仍须独立人或严格单管理员确认。旧直接 paid / failed 接口已拒绝。
- pending 可填原因拒绝；提交满 24 小时仍未终结标为超时，统计按申请时间计算，不自动交接、释放或退款。每次状态操作绑定当前版本，过期重试要求刷新，不会重复登记、付款或释放。
- 总余额 = 结算中 + 可提现 + 预留 + 已提现；累计赚取 = 可提现 + 预留 + 已提现（不含结算中）。历史提现流水和已提现金额原样保留，不倒推为新申请。

普通列表仅返回脱敏收款信息；完整收款信息、逐笔时间线和私有凭证仅管理员经授权读取。凭证以提现密钥加密存储，不返回对象路径或匿名下载链接；下载强制附件、no-store、nosniff 并审计。团长不能读取提现私密信息。审计记录交接前后身份和资金变化，不记录完整收款账户；原因 / 参考号不得填写敏感账号或姓名。
...
先备份数据库及私有对象存储并验证恢复，再按既有迁移命令依序运行 `ManualPayouts2026092000001`、已部署的次日结算迁移、`PayoutSupervision2026092200001`、`PayoutEvidence2026092300001`。监管迁移不改钱包余额；原 processing 从批次创建人推导经办人，缺失 / 停用时必须显式交接。旧 paid 标记 legacy，不虚构凭证或独立复核；旧审计导入逐笔时间线。次日结算及监管迁移仅前进，禁止生产 migration down；回退必须恢复核验过的备份，不清空或重建原有资金申请。
