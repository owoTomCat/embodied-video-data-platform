...
当前版本保留了原有 Web 界面，账号、登录会话、团队权限、账户审计、视频上传、AI 质检、人工复核写回、积分周期锁定和交付包均已迁移到本地 NestJS + PostgreSQL 后端。视频文件由浏览器分片直传 MinIO，经 RabbitMQ 先后进入独立媒体进程和 AI 质检进程；媒体进程使用 FFprobe/FFmpeg 提取证据，AI 进程使用 Qwen3.7 完成初检、条件复核和服务端规则复算。提现采用管理员线下自行转账、平台勾选已打款的方式，不连接支付 API。
...
- 用户可在 `/account/profile` 的「常用收款信息」分别保存一份支付宝和银行账户，填写各自的收款人姓名、账号，银行账户还需银行名称。可以独立修改或删除；资料以现有提现密钥加密存储，不进入公开账号资料或浏览器本地存储。
- 提现默认仍为手动填写，也可从「收款信息来源」选用已保存资料，填入后仍能修改本次内容，不自动回写个人资料。常用资料加载失败不阻断手动填写；修改、删除常用资料不会改变任何已提交申请的收款快照。
- `GET /api/v1/wallet/recipients`、`PUT /api/v1/wallet/recipients/:method`、`DELETE /api/v1/wallet/recipients/:method` 仅操作当前有效登录用户本人的资料；`method` 为 `alipay` 或 `bank`。接口禁止缓存，写入校验来源，审计不记录收款明文。
- 管理员在 `/admin/withdrawals` 表格直接看到申请人、团队、金额和完整收款姓名、账号、银行资料。默认显示未打款，支持已打款 / 全部及服务端跨页搜索申请 ID、用户 ID、用户名、姓名和团队。
- 表格固定表头与右侧勾选列；账号可完整复制，复制和保存均有结果反馈。支持每页 20 / 50 / 100 条，底部金额仅汇总当前页，不代表全部待打款余额。刷新保留当前资料但暂时禁用勾选，避免在刷新过程中操作旧记录；窄屏可横向浏览完整资料。
- 管理员核对后在线下自行转账，再在该行勾选「已打款」。不需要领取、交接、登记参考号、上传凭证、复核或选择单人模式；平台不自动转账，也不验证银行实际到账。
- `GET /api/v1/wallet/payouts` 仅有效管理员可访问，完整资料读取留审计且禁止缓存。`POST /api/v1/wallet/payouts/:id/confirm` 无需表单；任何有效管理员均可确认未终结申请，包括历史 processing / review_pending / investigating。历史状态不代表可以再次转账，操作前须线下核对。
- 勾选原子地把预留转为已提现 / 累计提现，只生成一笔 withdraw 流水；重复点击、并发点击返回首次确认的管理员和时间，不重复扣款。已打款不可取消或覆盖；历史拒绝 / 失败也不能再次确认。
- 记录申请时间、确认管理员账号和确认时间。`confirmedAt` 是平台勾选时间，**不是银行转账时间**；历史已支付优先展示原确认记录，不虚构缺失时间。旧领取、导出、交接、登记、审核、调查、退款与凭证上传接口已移除。
- 总余额 = 结算中 + 可提现 + 预留 + 已提现；累计赚取 = 可提现 + 预留 + 已提现（不含结算中）。历史提现流水和已提现金额原样保留，不倒推为新申请。

数采普通列表仅返回脱敏收款信息；完整表格、历史逐笔时间线和历史私有凭证仅管理员经授权读取。历史凭证保留只读下载，以原提现密钥解密，不返回对象路径或匿名下载链接；下载强制附件、no-store、nosniff 并审计。团长不能读取提现私密信息。审计保留操作者和资金变化，但不记录收款明文或搜索内容。

### 提现密钥与部署

先备份数据库及私有对象存储并验证恢复，再按既有迁移命令顺序运行尚未执行的迁移，包括新增 `SimplePayouts2026092400001`；不修改或重跑已部署的旧迁移。新增迁移仅放宽直接确认所需的批次 / 付款登记约束并允许 manual 确认，不改钱包余额、原申请状态、密文、批次、登记、审计或凭证。原 processing 无需交接即可由有效管理员勾选；迁移本身绝不确认或退款。旧 paid 与历史确认信息原样保留。财务迁移仅前进，禁止生产 migration down；回退必须恢复核验过的备份，不清空或重建原有资金申请。
常用收款资料功能还需执行新增 `SavedPayoutRecipients2026092500001`，仅创建 `saved_payout_recipients` 表，不回填旧申请、不修改余额或历史收款快照。沿用原 `PAYOUT_RECIPIENT_KEY`；上线前备份并在恢复库验证新增迁移。
...
通过受控环境 / secret manager 配置到 API（现有 Compose 的 API `env_file: .env` 会加载它），不要提交到仓库或复制到浏览器环境变量。无密钥或格式错误时提交、完整表格读取与确认返回不可用，绝不保存明文；余额查询仍可用。收款信息采用 AES-256-GCM，随机 IV，并绑定申请 ID 防止替换密文；幂等信息使用密钥 HMAC，避免泄露低熵账号。

**密钥须与数据库备份分别安全保存并测试恢复。** 密钥丢失无法解密历史收款信息；不要运行环境重建时生成替代密钥，也不要在已有申请时直接替换它。当前版本不提供在线密钥轮换接口；轮换须先停用申请及完整资料访问，制定受控解密重加密与一致性验证方案后操作。`deploy/prepare-production-env.sh` 不生成或覆盖此密钥，部署人员须显式提供原密钥。未配置密钥时可部署非提现功能，但不能验收提现提交、收款表格和打款确认。
